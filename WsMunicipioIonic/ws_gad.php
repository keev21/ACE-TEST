...
if ($post['accion'] == 'guardar_inventario') {
    ...
    // Verificar si ya existe un registro con el mismo producto, fecha y tipo de precio
    $stmt_verificar = mysqli_prepare($mysqli, "SELECT RI_CODIGO FROM inventario_registro_inicial WHERE PROD_CODIGO = ? AND RI_FECHA = ? AND RI_TIPOPRECIO = ?");
    mysqli_stmt_bind_param($stmt_verificar, "iss", $producto_id, $fecha_registro, $tipo_precio);
    mysqli_stmt_execute($stmt_verificar);
    mysqli_stmt_store_result($stmt_verificar);

    if (mysqli_stmt_num_rows($stmt_verificar) > 0) {
        // Ya existe un registro, enviar mensaje de error
        $respuesta = json_encode(array('estado' => false, 'mensaje' => 'El inventario ya está guardado para este producto, fecha y tipo de precio.'));
    } else {
        // No existe un registro, proceder a guardar
